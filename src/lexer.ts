import { delimOperators } from "./operators";
import Token, { TokenTypes } from "./token";

export default class Lexer {
  private index: number = 0;
  private readonly lastIndex: number = this.source.length - 1;
  tokens: Token[] = [];
  ...
  readString(
    ...
    quote: TokenTypes.SINGLE_QUOTES | TokenTypes.DOUBLE_QUOTES
  ) {
    let token = [];
